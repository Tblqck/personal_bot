...
from upload_pending_tasks import upload_pending_tasks  # we will modify this to accept silent=True
...
def trigger_background_upload():
    try:
        threading.Thread(target=lambda: upload_pending_tasks(silent=True), daemon=True).start()
    except Exception:
        pass
...
def save_task(task):
    ...
    updated = False
    for r in rows:
        ratio = SequenceMatcher(None, (r.get("title") or "").lower(), (task.get("title") or "").lower()).ratio()
...
def load_user_tasks(user_id):
    ...
    # sort by closest due first + most recent added
    def task_sort_key(t):
        try:
    ...
    return tasks_sorted[:30]  # 20 closest + 10 most recent roughly
...
def find_best_task_match(user_id, title):
    ...
    for t in load_user_tasks(user_id):
        r = SequenceMatcher(None, (t.get("title") or "").lower(), title.lower()).ratio()
        if r > best_ratio:
            best_ratio = r
            best = t
...
def save_chat_context(user_id, role, message, max_per_user=40):
    ...
    if os.path.exists(CONTEXT_CSV):
        with open(CONTEXT_CSV, newline="", encoding="utf-8") as f:
            rows = list(csv.DictReader(f))
    rows.append({"user_id": uid, "timestamp": timestamp, "role": role, "message": message})
    ...
    with open(CONTEXT_CSV, newline="", encoding="utf-8") as f:
        for r in csv.DictReader(f):
            ...
            if (now - ts).total_seconds() <= max_age_minutes * 60:
                out.append({"role": r["role"], "content": r["message"]})
    return out
...
def parse_message_with_gpt(message, user_id=None):
    ...
    try:
        resp = openai.chat.completions.create(model="gpt-4", messages=messages, temperature=0.2)
        content = resp.choices[0].message.content.strip()
        start = content.find("{")
        end = content.rfind("}") + 1
        result = json.loads(content[start:end])
    except Exception:
        result = {"action":"chat","title":None,"details":None,"due":None,"list_scope":None,"ai_comment":"","response_text":"Sorry, I didn't get that."}
